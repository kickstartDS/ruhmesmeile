...
| Deploy | push to `main` → CircleCI `kamal deploy -c config/deploy-website.yml` (branch-filtered to `main`); from a workstation, source `packages/website/.env` + `.env.local` first so the ERB in `config/deploy-website.yml` resolves |
