...
- **Subheadline sizes.** The old design system re-applied a per-instance `--subheadline--font` (a `font` shorthand carrying size *and* line-height) that came from the theme, so a homepage hero subheadline rendered at `--ks-font-copy-l` (23.805px/1.5). The redesign dropped both the shorthand rule and any source for that variable, so subheadlines now take the level's `em` size (that hero: 33.28px/1.15). Restoring the rule alone does not help — the variable is set nowhere in the current build — and every subheadline inherits the same treatment, so a fix has to come from whatever carries the per-instance setting.
- **Storytelling cta right padding.** A new container-query rule zeroes the box's right padding alongside the image (a new layout mechanism). Top and left insets, and the padding shorthand, are restored; the text column measures 643px against the legacy 603px (it was 700px before).
